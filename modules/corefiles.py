...
def initialize_json(file_path: str) -> None:
    """
    Inicializa el archivo con la estructura base de cuentasBancarias si no existe
    """
    estructura_inicial = {
        "cuentasBancarias": {}
    }
    ...
    if not os.path.isfile(file_path):
        write_json(file_path, estructura_inicial)
    else:
        current_data = read_json(file_path)
        if "cuentasBancarias" not in current_data:
            current_data["cuentasBancarias"] = {}
        write_json(file_path, current_data)